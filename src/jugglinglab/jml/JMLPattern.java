...
public class JMLPattern {
	...
	protected String version = JMLDefs.default_JML_on_save; // JML version
															// number
	...
	protected boolean[][] hasVDHandJMLTransition; // whether pattern has a
													// velocity-defining
													// transition,
	...
	// Used to specify the jml version number, when pattern is part of a
	// patternlist
	...
	public JMLPattern(Reader read) throws JuggleExceptionUser, SAXException,
			IOException {
		this();
	...
	public void addPosition(JMLPosition pos) throws JuggleExceptionUser {
		if ((pos.getT() < getLoopStartTime())
				|| (pos.getT() > getLoopEndTime()))
			return; // throw new
					// JuggleExceptionUser("<position> time out of range");
	...
	public void layoutPattern() throws JuggleExceptionInternal,
			JuggleExceptionUser {
		if (!valid)
			throw new JuggleExceptionInternal(
					"Cannot do layout of invalid pattern");
		...
		if (jugglinglab.core.Constants.DEBUG_LAYOUT) {
			for (int i = 0; i < getNumberOfPaths(); i++) {
				System.out.println(pathlinks[i].size() + " pathlinks for path "
						+ (i + 1) + ":");
				for (int jtemp = 0; jtemp < pathlinks[i].size(); jtemp++)
					System.out.println("   "
							+ (PathLink) (pathlinks[i].elementAt(jtemp)));
			}
			for (int i = 0; i < getNumberOfJugglers(); i++) {
				for (int j = 0; j < 2; j++) {
					System.out.println(handlinks[i][j].size()
							+ " handlinks for juggler " + (i + 1) + ", hand "
							+ (j + 1) + ":");
					for (int k = 0; k < handlinks[i][j].size(); k++)
						System.out.println("   "
								+ (HandLink) (handlinks[i][j].elementAt(k)));
				}
			}
		}
		laidout = true;
	...
	public void buildEventList() throws JuggleExceptionInternal,
			JuggleExceptionUser {
		// figure out how many events there are
		int numevents = 0;
		JMLEvent current = eventlist;
		while (current != null) {
			if ((current.getJuggler() < 1)
					|| (current.getJuggler() > numjugglers))
				throw new JuggleExceptionUser(
						errorstrings.getString("Error_juggler_outofrange"));
			if (current.isMaster())
				numevents++;
			else
				removeEvent(current);
		...
		for (int i = 0; i < numjugglers; i++) {
			...
			for (int j = 0; j < numevents; j++) {
				if (hasJMLTransitionForLeft == false)
					hasJMLTransitionForLeft = ei[j].hasJMLTransitionForHand(
							i + 1, HandLink.LEFT_HAND);
				if (hasJMLTransitionForRight == false)
					hasJMLTransitionForRight = ei[j].hasJMLTransitionForHand(
							i + 1, HandLink.RIGHT_HAND);
				if (hasVDHandJMLTransition[i][0] == false)
					hasVDHandJMLTransition[i][0] = ei[j]
							.hasVDJMLTransitionForHand(i + 1,
									HandLink.LEFT_HAND);
				if (hasVDHandJMLTransition[i][1] == false)
					hasVDHandJMLTransition[i][1] = ei[j]
							.hasVDJMLTransitionForHand(i + 1,
									HandLink.RIGHT_HAND);
			}
			if (hasJMLTransitionForLeft == false) {
				String template = errorstrings
						.getString("Error_no_left_events");
				Object[] arguments = { new Integer(i + 1) };
				throw new JuggleExceptionUser(MessageFormat.format(template,
						arguments));
			}
			if (hasJMLTransitionForRight == false) {
				String template = errorstrings
						.getString("Error_no_right_events");
				Object[] arguments = { new Integer(i + 1) };
				throw new JuggleExceptionUser(MessageFormat.format(template,
						arguments));
			}
			needVDHandEvent[i][0] = hasVDHandJMLTransition[i][0]; // set up for
																	// later
		...
		for (int i = 0; i < numpaths; i++) {
			...
			for (int j = 0; j < numevents; j++) {
				...
				if (hasVDPathJMLTransition[i] == false)
					hasVDPathJMLTransition[i] = ei[j]
							.hasVDJMLTransitionForPath(i + 1);
			}
			if (hasPathJMLTransition == false) {
				String template = errorstrings
						.getString("Error_no_path_events");
				Object[] arguments = { new Integer(i + 1) };
				throw new JuggleExceptionUser(MessageFormat.format(template,
						arguments));
			}
				...
				for (int i = 0; i < maxevent.getNumberOfTransitions(); i++) {
					...
					case JMLTransition.TRANS_SOFTCATCH:
						if (needVDHandEvent[jug][han] == true)
							needSpecialPathEvent[path] = true; // need
																// corresponding
																// throw to get
																// velocity
						needVDHandEvent[jug][han] = needHandEvent[jug][han] = false;
					...
					case JMLTransition.TRANS_HOLDING:
						if (hasVDPathJMLTransition[path] == false) // if no
																	// throws
																	// for this
																	// path,
																	// then done
		...
		do {
			...
			if (mintime > this.getLoopEndTime()) {
				...
				// if this hand has no throws/catches, then need to build out
				// event list
				// past a certain time, due to how the hand layout is done in
				// this case
	...
	public void findMasterEvents() throws JuggleExceptionInternal,
			JuggleExceptionUser {
	...
	public void findPositions() throws JuggleExceptionInternal {
		...
		this.jugglerangle = ((Constants.ANGLE_LAYOUT_METHOD == Curve.lineCurve) ? (Curve[]) (new lineCurve[this
				.getNumberOfJugglers()]) : (Curve[]) (new splineCurve[this
				.getNumberOfJugglers()]));
		...
		for (int i = 1; i <= this.getNumberOfJugglers(); i++) {
			...
			if (num == 0) {
				...
				jugglerangle[i - 1] = ((Constants.ANGLE_LAYOUT_METHOD == Curve.lineCurve) ? (Curve) (new lineCurve())
				...
				if (this.getNumberOfJugglers() == 1) {
					...
					positions[0].setCoordinate(
							r
									* Math.cos(JLMath.toRad(theta
											* (double) (i - 1))),
							r
									* Math.sin(JLMath.toRad(theta
											* (double) (i - 1))), 100.0);
					angles[0].setCoordinate(90.0 + theta * (double) (i - 1),
							0.0, 0.0);
				}
				...
				times[num] = times[0] + this.getLoopEndTime()
						- this.getLoopStartTime();
	...
	// HandLink lists
	protected void buildLinkLists() throws JuggleExceptionUser,
		...
		for (i = 0; i < getNumberOfPaths(); i++) {
				...
				if (lastev != null) {
					...
					case JMLTransition.TRANS_HOLDING:
						if (lasttr.getType() == JMLTransition.TRANS_THROW) {
							String template = errorstrings
									.getString("Error_successive_throws");
							Object[] arguments = { new Integer(i + 1) };
						...
						if (lastev.getJuggler() != ev.getJuggler()) {
							String template = errorstrings
									.getString("Error_juggler_changed");
							Object[] arguments = { new Integer(i + 1) };
							throw new JuggleExceptionUser(MessageFormat.format(
									template, arguments));
						}
					...
					case JMLTransition.TRANS_CATCH:
						if (lasttr.getType() != JMLTransition.TRANS_THROW) {
							String template = errorstrings
									.getString("Error_successive_catches");
							Object[] arguments = { new Integer(i + 1) };
							throw new JuggleExceptionUser(MessageFormat.format(
									template, arguments));
						}
			...
			if (pathlinks[i].size() == 0)
				throw new JuggleExceptionInternal("No event found for path "
						+ (i + 1));
		}
		...
		for (i = 0; i < getNumberOfJugglers(); i++) {
			...
			for (j = 0; j < 2; j++) {
				int handnum = (j == 0 ? HandLink.LEFT_HAND
						: HandLink.RIGHT_HAND);
				...
				done2: while (true) {
					...
					if (lastev != null) {
						...
						hl.setStartVelocityRef(lastvr); // may be null, which is
														// ok
	...
	protected void layoutHandPaths() throws JuggleExceptionInternal {
		...
		// go through HandLink lists, creating Path objects and calculating
		// paths

		for (int i = 0; i < getNumberOfJugglers(); i++) {
			for (int j = 0; j < 2; j++) {
				// There are two cases -- a hand has throw or softcatch events
				// (which define
				// hand velocities at points in time), or it does not (no
				// velocities known).
				// To determine the spline paths, we need to solve for hand
				// velocity at each
				...
				if (hasVDHandJMLTransition[i][j]) {
					...
					for (int k = 0; k < handlinks[i][j].size(); k++) {
						...
						if (hl.getStartVelocityRef() != null) {
							// this is guaranteed to happen before the loop
							// start time, given
							// the way we built the event list above
							startlink = hl;
							num = 1;
						}
						if ((hl.getEndVelocityRef() != null)
								&& (startlink != null)) {
							...
							for (int l = 0; l < num; l++) {
								HandLink hl2 = (HandLink) handlinks[i][j]
										.elementAt(k - num + 1 + l);
								pos[l] = hl2.getStartEvent()
										.getGlobalCoordinate();
							...
							Coordinate startvel = startlink
									.getStartVelocityRef().getVelocity();
							Coordinate endvel = hl.getEndVelocityRef()
									.getVelocity();
					...
					// Build chain and solve for velocities. This implementation
					// is a little
					// inefficient since it builds the second chain by a
					// duplicate calculation rather
					// than a copy. Sketch of algorithm:
					// find first handlink that straddles loopStartTime -- call
					// it startlink
					// startevent = first event in startlink
					// delayedstartevent = corresponding event 1 delay period
					// after startevent
					// find handlink that ends with delayedstartevent -- call it
					// endlink
					// build spline hand path from startlink to endlink, and
					// calculate (chain 1)
					// startlink = next link after endlink
					// delayed2startevent = corresponding event 1 delay period
					// after delayedstartevent
					// find handlink that ends with delayed2startevent -- call
					// it endlink
					// build spline hand path from startlink to endlink, and
					// calculate (chain 2)
					int k;
					...
					for (int chain = 0; chain < 2; chain++) {
						...
						pos[num] = hl.getEndEvent().getGlobalCoordinate();
						times[num] = hl.getEndEvent().getT();
						hp.setCurve(pos, times, null, null); // null endpoint
																// velocities
																// signals to
																// calculate
						hp.calcCurve();
	...
	// ------------------------------------------------------------------------
	// Methods used by animator to get prop and body locations at specified
	// times.
	// ------------------------------------------------------------------------
	...
	public JMLSymmetry getSymmetry(int index) {
		return (JMLSymmetry) symmetries.elementAt(index);
	}
	...
	public void getPathCoordinate(int path, double time, Coordinate newPosition)
		...
		throw new JuggleExceptionInternal("time t=" + time
	...
	public void getJugglerPosition(int juggler, double time,
	...
	public Coordinate convertLocalToGlobal(Coordinate lc, int juggler,
			double time) {
		...
		Coordinate gc = new Coordinate(origin.x + lc.x * Math.cos(angle) - lc.y
				* Math.sin(angle), origin.y + lc.x * Math.sin(angle) + lc.y
				* Math.cos(angle), origin.z + lc.z);
		return gc;
	}
	...
	public Coordinate convertGlobalToLocal(Coordinate gc, int juggler, double t) {
		...
		Coordinate lc = new Coordinate(c2.x * Math.cos(angle) + c2.y
				* Math.sin(angle), -c2.x * Math.sin(angle) + c2.y
				* Math.cos(angle), c2.z);
		lc.y -= Juggler.pattern_y;
		return lc;
	}
	...
	public void getHandCoordinate(int juggler, int hand, double time,
			Coordinate newPosition) throws JuggleExceptionInternal {
		int handindex = (hand == HandLink.LEFT_HAND) ? 0 : 1;

		for (int i = 0; i < handlinks[juggler - 1][handindex].size(); i++) {
			HandLink hl = (HandLink) handlinks[juggler - 1][handindex]
					.elementAt(i);
			if ((time >= hl.getStartEvent().getT())
				...
				if (hp == null)
					throw new JuggleExceptionInternal(
							"getHandCoordinate() null pointer");
				hl.getHandCurve().getCoordinate(time, newPosition);
				return;
			}
		}
		throw new JuggleExceptionInternal("time t=" + time + " (j=" + juggler
				+ ",h=" + handindex + ") is out of handpath range");
	}

	// Get volume of any catch made between time1 and time2; if no catch,
	// returns 0.0
	public double getPathThrowVolume(int path, double time1, double time2) {
		int i;
		...
		for (i = 0; i < pathlinks[path - 1].size(); i++) {
			...
			if ((time1 >= pl1.getStartEvent().getT())
					&& (time1 <= pl1.getEndEvent().getT()))
				break;
		}
		...
		while (true) {
			...
			if ((time2 >= pl2.getStartEvent().getT())
					&& (time2 <= pl2.getEndEvent().getT()))
				break;
		...
		// Java 1.2 doesn't provide a way to adjust the playback volume of an
		// audio clip,
	...
	// Get volume of any bounce between time1 and time2; if no catch, returns
	// 0.0
	...
	public Coordinate getHandMax(int juggler, int hand) {
		...
		for (int i = 0; i < handlinks[juggler - 1][handnum].size(); i++) {
			HandLink hl = (HandLink) handlinks[juggler - 1][handnum]
					.elementAt(i);
			Curve hp = hl.getHandCurve();
			if (hp != null)
	...
	protected void printEventList() {
		...
		while (current != null) {
			if (current.isMaster())
				...
				System.out.println("  Slave event; master at t="
						+ current.getMaster().getT());
	...
	protected void readJML(JMLNode current) throws JuggleExceptionUser {
		...
		if (type.equalsIgnoreCase("jml")) {
			...
			try {
				...
				throw new JuggleExceptionUser(
						errorstrings.getString("Error_setup_tag"));
			}
			...
			if (propstring != null) {
				...
				if (st.countTokens() != numpaths)
					throw new JuggleExceptionUser(
							errorstrings.getString("Error_prop_assignments"));
				try {
					for (int i = 0; i < numpaths; i++) {
						pa[i] = Integer.valueOf(st.nextToken()).intValue();
						if ((pa[i] < 1) || (pa[i] > getNumberOfProps()))
							throw new JuggleExceptionUser(
									errorstrings.getString("Error_prop_number"));
					}
				} catch (NumberFormatException nfe) {
					throw new JuggleExceptionUser(
							errorstrings.getString("Error_prop_format"));
				}
			} else {
				for (int i = 0; i < numpaths; i++)
					pa[i] = 1;
			...
			ev.readJML(current, loadingversion, getNumberOfJugglers(),
					getNumberOfPaths()); // look at subnodes
			this.addEvent(ev);
	...
	public void writeJML(Writer wr, boolean title) throws IOException {
		...
		if (title)
			write.println("<title>" + JMLNode.xmlescape(this.title)
					+ "</title>");
		for (int i = 0; i < props.size(); i++)
			((PropDef) props.elementAt(i)).writeJML(write);

		String out = "<setup jugglers=\"" + getNumberOfJugglers()
				+ "\" paths=\"" + getNumberOfPaths() + "\" props=\""
				+ getPropAssignment(1);
