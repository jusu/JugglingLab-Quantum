	...
	public static Path getPath(String name) throws JuggleExceptionUser {
		...
		try {
			Object obj = Class.forName(
					"jugglinglab.path." + name.toLowerCase() + "Path")
					.newInstance();
			// if (obj == null)
			// throw new
			// JuggleExceptionUser("Cannot create Path type '"+name+"'");
			if (!(obj instanceof Path))
				throw new JuggleExceptionUser("Path type '" + name
						+ "' doesn't work");
			return (Path) obj;
		} catch (ClassNotFoundException cnfe) {
			throw new JuggleExceptionUser("Path type '" + name + "' not found");
		} catch (IllegalAccessException iae) {
			throw new JuggleExceptionUser("Cannot access '" + name
					+ "' path file (security)");
