...
public class Renderer2D extends Renderer {
	...
	public void initDisplay(Dimension dim, int border, Coordinate overallmax,
			Coordinate overallmin) {
		this.width = dim.width;
		this.height = dim.height;
		Rectangle r = new Rectangle(border, border, width - 2 * border, height
				- 2 * border);
	...
	public void setCameraAngle(double[] camangle) {
		...
		m = JLMatrix.shiftMatrix(-cameracenter.x, -cameracenter.y,
				-cameracenter.z);
		m.transform(JLMatrix.rotateMatrix(0.0, JLMath.toRad(180.0)
				- cameraangle[0], 0.0));
		m.transform(JLMatrix.rotateMatrix(JLMath.toRad(90.0) - cameraangle[1],
				0.0, 0.0));
		m.transform(JLMatrix.shiftMatrix(cameracenter.x, cameracenter.y,
				cameracenter.z));

		m.transform(JLMatrix.scaleMatrix(1.0, -1.0, 1.0)); // larger y values ->
															// smaller y pixel
															// coord
	...
	public void drawFrame(double time, int[] pnum, Graphics g, JPanel pan)
		...
		for (int i = 1; i <= pat.getNumberOfJugglers(); i++) {
			...
			getXYZ(jugglervec[i - 1][8], obj[index].coord[4]); // left head
																// bottom
			getXYZ(jugglervec[i - 1][9], obj[index].coord[5]); // left head top
			getXYZ(jugglervec[i - 1][10], obj[index].coord[6]); // right head
																// bottom
			getXYZ(jugglervec[i - 1][11], obj[index].coord[7]); // right head
																// top
			...
			for (int j = 0; j < 2; j++) {
				if (jugglervec[i - 1][4 + j] == null) {
					...
					obj[index].number = i;
					getXYZ(jugglervec[i - 1][2 + j], obj[index].coord[0]); // entire
																			// arm
					...
					obj[index].type = DrawObject2D.TYPE_LINE;
					obj[index].number = i;
					getXYZ(jugglervec[i - 1][2 + j], obj[index].coord[0]); // upper
																			// arm
					...
					obj[index].type = DrawObject2D.TYPE_LINE;
					obj[index].number = i;
					getXYZ(jugglervec[i - 1][4 + j], obj[index].coord[0]); // lower
																			// arm
					getXYZ(jugglervec[i - 1][0 + j], obj[index].coord[1]);
		...
		while (changed) {
			...
			for (int i = 0; i < numobjects; i++) {
				...
				for (int j = 0; j < obj[i].covering.size(); j++) {
					DrawObject2D temp = (DrawObject2D) (obj[i].covering
							.elementAt(j));
					if (!temp.drawn) {
						candraw = false;
						break;
		...
		for (int i = 0; i < numobjects; i++) {
			...
			case DrawObject2D.TYPE_PROP:
				...
				if (propimage != null) {
					...
					if (i == 0) {
						// XY COORDS
						// System.out.println("pop0 " + (x-grip.width) + " " +
						// (y-grip.height));
					}
				} /*
				 * else { g.setColor(pr.getEditorColor()); draw3DProp(ob.object,
				 * g); }
				 */
				...
				// g.drawLine(ob.boundingbox.x + ob.boundingbox.width,
				// ob.boundingbox.y, ob.boundingbox.x + ob.boundingbox.width,
				// ob.boundingbox.y + ob.boundingbox.height);
				// g.drawLine(ob.boundingbox.x + ob.boundingbox.width,
				// ob.boundingbox.y + ob.boundingbox.height, ob.boundingbox.x,
				// ob.boundingbox.y + ob.boundingbox.height);
				// g.drawLine(ob.boundingbox.x, ob.boundingbox.y +
				// ob.boundingbox.height, ob.boundingbox.x, ob.boundingbox.y);
			...
			case DrawObject2D.TYPE_BODY:
				...
				for (int j = 0; j < polysides; j++) {
					headx[j] = (int) (0.5f + 0.5f * (LheadBx + RheadBx + headcos[j]
							* (RheadBx - LheadBx)));
					heady[j] = (int) (0.5f + 0.5f * (LheadBy + LheadTy + headsin[j]
							* (LheadBy - LheadTy)) + (headx[j] - LheadBx)
							* (RheadBy - LheadBy) / (RheadBx - LheadBx));
				}
			...
			// g.drawLine(ob.boundingbox.x+ob.boundingbox.width-1,
			// ob.boundingbox.y, ob.boundingbox.x+ob.boundingbox.width-1,
			// ob.boundingbox.y+ob.boundingbox.height-1);
			// g.drawLine(ob.boundingbox.x+ob.boundingbox.width-1,
			// ob.boundingbox.y+ob.boundingbox.height-1, ob.boundingbox.x,
			// ob.boundingbox.y+ob.boundingbox.height-1);
			// g.drawLine(ob.boundingbox.x,
			// ob.boundingbox.y+ob.boundingbox.height-1, ob.boundingbox.x,
			// ob.boundingbox.y);
	...
	public Coordinate getJugglerWindowMax() {
		...
		max = Coordinate.add(max, new Coordinate(Juggler.shoulder_hw,
				Juggler.shoulder_hw, // Juggler.head_hw,
				Juggler.shoulder_h + Juggler.neck_h + Juggler.head_h));
		return max;
		// return new Coordinate(Math.max(max.x, max.y), Math.max(max.x, max.y),
		// max.z);
	}
	...
	public Coordinate getJugglerWindowMin() {
		...
		min = Coordinate.add(min, new Coordinate(-Juggler.shoulder_hw,
				-Juggler.shoulder_hw, // -Juggler.head_hw,
				Juggler.shoulder_h));
		return min;
		// return new Coordinate(Math.min(min.x, min.y), Math.min(min.x, min.y),
		// min.z);
	}

	protected void calcScaling(Rectangle r, Coordinate coordmax,
			Coordinate coordmin) {
		// double frame_width = 2.0 * Math.max(Math.abs(coordmax.x),
		// Math.abs(coordmin.x));
		double frame_width = coordmax.x - coordmin.x;
		double frame_height = coordmax.z - coordmin.z;
		zoom = Math.min((double) (r.width) / frame_width, (double) (r.height)
				/ frame_height);
		originx = r.x
				+ (int) (0.5 + 0.5 * (r.width - zoom
						* (coordmax.x + coordmin.x))); // r.x + r.width / 2;
		originz = r.y
				+ (int) (0.5 + 0.5 * (r.height + zoom
						* (coordmax.z + coordmin.z)));
	}
	...
	class DrawObject2D {
		...
		public boolean isCovering(DrawObject2D obj) {
			...
			case TYPE_PROP:
				...
				case TYPE_BODY: {
					vectorProduct(obj.coord[0], obj.coord[1], obj.coord[2],
							tempv);
					if (tempv.z == 0f)
						return false;
					double z = obj.coord[0].z
							- (tempv.x * (this.coord[0].x - obj.coord[0].x) + tempv.y
									* (this.coord[0].y - obj.coord[0].y))
							/ tempv.z;
			...
			case TYPE_BODY:
				switch (obj.type) {
				case TYPE_PROP: {
					vectorProduct(this.coord[0], this.coord[1], this.coord[2],
							tempv);
					if (tempv.z == 0f)
						return false;
					double z = this.coord[0].z
							- (tempv.x * (obj.coord[0].x - this.coord[0].x) + tempv.y
									* (obj.coord[0].y - this.coord[0].y))
							/ tempv.z;
		...
		protected int isBoxCoveringLine(DrawObject2D box, DrawObject2D line) {
			// If at least one end of the line is inside the box's boundingbox,
			// then return
			// 1 if all such ends are behind the box, and -1 otherwise.
			// If neither end is inside the boundingbox, then find intersections
			// between the
			// line and the boundingbox. If no points of intersection, return 0.
			// If the
			// line is behind the bb at all points of intersection, return 1.
			// Otherwise
			// return -1;
			...
			for (int i = 0; i < 2; i++) {
				...
				if (box.boundingbox
						.contains((int) (x + 0.5f), (int) (y + 0.5f))) {
					double zb = box.coord[0].z
							- (tempv.x * (x - box.coord[0].x) + tempv.y
									* (y - box.coord[0].y)) / tempv.z;
			...
			for (int i = 0; i < 2; i++) {
				int x = ((i == 0) ? box.boundingbox.x : (box.boundingbox.x
						+ box.boundingbox.width - 1));
				...
				if ((y < box.boundingbox.y)
						|| (y > (box.boundingbox.y + box.boundingbox.height - 1)))
					continue;
				intersection = true;
				double zb = box.coord[0].z
						- (tempv.x * (x - box.coord[0].x) + tempv.y
			...
			for (int i = 0; i < 2; i++) {
				int y = ((i == 0) ? box.boundingbox.y : (box.boundingbox.y
						+ box.boundingbox.height - 1));
				...
				if ((x < box.boundingbox.x)
						|| (x > (box.boundingbox.x + box.boundingbox.width - 1)))
					continue;
				intersection = true;
				double zb = box.coord[0].z
						- (tempv.x * (x - box.coord[0].x) + tempv.y
								* ((double) y - box.coord[0].y)) / tempv.z;
				double zl = line.coord[0].z
						+ (line.coord[1].z - line.coord[0].z)
						* (x - line.coord[0].x)
						/ (line.coord[1].x - line.coord[0].x);
		...
		public JLVector vectorProduct(JLVector v1, JLVector v2, JLVector v3,
				JLVector result) {
			double ax = v2.x - v1.x;
			double ay = v2.y - v1.y;
			double az = v2.z - v1.z;
