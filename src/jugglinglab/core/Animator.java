...
public class Animator extends JPanel implements Runnable {
	...
	protected void initHandlers() {
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent me) {
				...
				// The following (and the equivalent in mouseReleased()) is a
				// hack to swallow
				// a mouseclick when the browser stops reporting enter/exit
				// events because the
				// user has clicked on something else. The system reports
				// simultaneous enter/press
				// events when the user mouses down in the component; we want to
				// swallow this as a
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent me) {
				...
				if (!cameradrag) {
					...
					Animator.this.camangle = Animator.this.ren1
							.getCameraAngle();
				}
	...
	public void restartJuggle(JMLPattern pat, AnimatorPrefs newjc)
			throws JuggleExceptionUser, JuggleExceptionInternal {
		// try to lay out new pattern first so that if there's an error we won't
		...
		remoteControl.setPattern(this.pat.getTitle());

		engine = new Thread(this);
		engine.start();
	}

	public void restartJuggle() throws JuggleExceptionUser,
		...
		try {
			...
			if (jc.mousePause) {
				if (outside_valid)
					...
					setPaused(true); // assume mouse is outside animator, if not
										// known
				waspaused = false;
				// waspaused_valid = true;
			}
			...
			engine = null; // this is critical as it signals restartJuggle()
							// that exit is occurring
		}
		synchronized (this) {
			notify(); // tell possible thread wait()ing in restartJuggle() that
						// animator thread is exiting
		}
	}
	...
	public void setTime(double time) {
		/*
		 * while (time < pat.getLoopStartTime()) time += (pat.getLoopEndTime() -
		 * pat.getLoopStartTime()); while (time > pat.getLoopEndTime()) time -=
	...
	protected void drawFrame(double sim_time, int[] pnum, Graphics g)
		...
		if (this.jc.stereo) {
			...
			this.ren2.drawFrame(sim_time, pnum,
					g.create(d.width / 2, 0, d.width / 2, d.height),
					Animator.this);
		} else {
...

		{
			...
			int yx = cx
					+ (int) (0.5 - xya
							* Math.cos(theta + 90.0 * 0.0174532925194));
			int yy = cy
					+ (int) (0.5 + xyb
							* Math.sin(theta + 90.0 * 0.0174532925194));
			int zx = cx;
	...
	public void syncToPattern() {
		...
		// figure out timing constants; adjust fps to get integer number of
		// frames in loop
		num_frames = (int) (0.5 + (pat.getLoopEndTime() - pat
				.getLoopStartTime()) * jc.slowdown * jc.fps);
		sim_interval_secs = (pat.getLoopEndTime() - pat.getLoopStartTime())
				/ num_frames;
	...
	protected void findMaxMin() {
		...
		// are
		// on which paths. We may also want to leave room for the rest of the
		// juggler.
		int i;
		...
		for (i = 1; i <= pat.getNumberOfJugglers(); i++) {
			handmax = Coordinate.max(handmax,
					pat.getHandMax(i, HandLink.LEFT_HAND));
			handmin = Coordinate.min(handmin,
					pat.getHandMin(i, HandLink.LEFT_HAND));
			handmax = Coordinate.max(handmax,
					pat.getHandMax(i, HandLink.RIGHT_HAND));
			handmin = Coordinate.min(handmin,
					pat.getHandMin(i, HandLink.RIGHT_HAND));
		}
		...
		if (pat.getNumberOfJugglers() == 1) {
			overallmin.z -= 0.3 * Math.max(Math.abs(overallmin.y),
					Math.abs(overallmax.y));
			overallmax.z += 5.0; // keeps objects from rubbing against top of
									// window
		} else {
			double tempx = Math.max(Math.abs(overallmin.x),
					Math.abs(overallmax.x));
			double tempy = Math.max(Math.abs(overallmin.y),
					Math.abs(overallmax.y));
	...
	protected void syncRenderer() {
		...
		if (this.jc.stereo) {
			d.width /= 2;
			this.ren1.initDisplay(d, jc.border, this.overallmax,
					this.overallmin);
			this.ren2.initDisplay(d, jc.border, this.overallmax,
					this.overallmin);
		} else
			this.ren1.initDisplay(d, jc.border, this.overallmax,
					this.overallmin);
	...
	public void writeGIFAnim() {
		try {
			...
			Method setup = jagw.getMethod("setup", new Class[] {
					Animator.class, jugglinglab.renderer.Renderer.class,
					jugglinglab.renderer.Renderer.class, Integer.TYPE,
					Double.TYPE, Long.TYPE });
			Object gw = jagw.newInstance();
			setup.invoke(gw, new Object[] { this, ren1, ren2,
					new Integer(num_frames), new Double(sim_interval_secs),
					new Long(real_interval_millis) });
