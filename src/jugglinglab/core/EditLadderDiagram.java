...
public class EditLadderDiagram extends LadderDiagram implements ActionListener {
	...
	public EditLadderDiagram(JMLPattern pat, JFrame parent) {
		...
		this.addMouseListener(new MouseAdapter() {
			public void mousePressed(final MouseEvent me) {
				...
				if (me.isPopupTrigger()) {
					...
					active_eventitem = getSelectedLadderEvent(me.getX(),
							me.getY());
					if (active_eventitem != null)
						popupitem = active_eventitem;
					else
						popupitem = getSelectedLadderPath(me.getX(), me.getY(),
								path_slop);
					popup_y = me.getY();
					if (animator != null) {
						double scale = (fpat.getLoopEndTime() - fpat
								.getLoopStartTime())
						...
						if (active_eventitem == null) {
							...
							if (animator != null) {
								double scale = (fpat.getLoopEndTime() - fpat
										.getLoopStartTime())
										/ (double) (height - 2 * border_top);
								double newtime = (double) (my - border_top)
										* scale;
								anim_paused = animator.getPaused();
								animator.setPaused(true);
								animator.setTime(newtime);
					...
					case STATE_MOVING_EVENT:
						// ErrorDialog.handleException(new
						// JuggleExceptionInternal("mouse pressed in MOVING_EVENT state"));
						break;
					case STATE_MOVING_TRACKER:
						// ErrorDialog.handleException(new
						// JuggleExceptionInternal("mouse pressed in MOVING_TRACKER state"));
						break;
					case STATE_POPUP:
			...
			public void mouseReleased(final MouseEvent me) {
				if (me.isPopupTrigger()) {
					...
					case STATE_EVENT_SELECTED:
					case STATE_MOVING_EVENT:
						// skip this code for MOVING_TRACKER state, since
						// already
					...
					case STATE_MOVING_TRACKER:
						...
						popupitem = active_eventitem;
						if (popupitem == null) {
							popupitem = getSelectedLadderEvent(me.getX(),
									me.getY());
							if (popupitem == null)
								popupitem = getSelectedLadderPath(me.getX(),
										me.getY(), path_slop);
						}
						adjustPopup(popupitem);
						popup.show(EditLadderDiagram.this, me.getX(), me.getY());
					...
					case STATE_POPUP:
						ErrorDialog
								.handleException(new JuggleExceptionInternal(
					...
					switch (gui_state) {
					case STATE_INACTIVE:
						// should only get here if user cancelled popup menu or
						// deselected event
						break;
					case STATE_EVENT_SELECTED:
					...
					case STATE_MOVING_EVENT:
						...
						if (delta_y != 0) {
							...
							for (int i = 0; i < laddereventitems.size(); i++) {
								LadderEventItem item = (LadderEventItem) laddereventitems
										.elementAt(i);
							...
							activeEventMoved();
							/*
							 * layoutPattern(); createView(); active_eventitem =
							 * null; if (animator != null)
							 * animator.deactivateEvent();
							 */
							repaint();
						}
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent me) {
				...
				switch (gui_state) {
				case STATE_INACTIVE:
					// This exception was being generated on popup cancelation
					// when it could have been ignored.
					// See bug report 861856.
					// ErrorDialog.handleException(new
					// JuggleExceptionInternal("mouse dragged in INACTIVE state"));
					break;
				case STATE_EVENT_SELECTED:
	...
	protected void findEventLimits(LadderEventItem item) {
		...
		for (int j = 0; j < item.event.getNumberOfTransitions(); j++) {
			...
			case JMLTransition.TRANS_THROW: {
				...
				while (ev != null) {
					if ((ev.getPathTransition(tr.getPath(),
							JMLTransition.TRANS_CATCH) != null)
							|| (ev.getPathTransition(tr.getPath(),
									JMLTransition.TRANS_SOFTCATCH) != null))
						break;
					ev = ev.getPrevious();
				}
				if (ev == null)
			...
			case JMLTransition.TRANS_SOFTCATCH: {
				...
				while (ev != null) {
					if (ev.getPathTransition(tr.getPath(),
							JMLTransition.TRANS_THROW) != null)
						break;
					ev = ev.getNext();
				}
				if (ev == null)
	...
	protected void moveEvent(LadderEventItem item) {
					...
					for (int j = 0; j < pat.getNumberOfPaths(); j++) {
						if (catchpath[j]) {
							JMLTransition tr = new JMLTransition(
									JMLTransition.TRANS_HOLDING, (j + 1), null,
									null);
							ev.addTransition(tr);
							if (!ev.isMaster()) {
								Permutation pp = ev.getPathPermFromMaster()
										.getInverse();
		...
		for (int j = 0; j < pat.getNumberOfPaths(); j++) {
			if (holdpathnew[j] != holdpathorig[j]) {
				if (holdpathnew[j]) {
					JMLTransition tr = new JMLTransition(
							JMLTransition.TRANS_HOLDING, pp.getMapping(j + 1),
							null, null);
					ev.addTransition(tr);
				} else {
					JMLTransition tr = ev.getPathTransition(
							pp.getMapping(j + 1), JMLTransition.TRANS_HOLDING);
					if (tr == null)
						ErrorDialog
								.handleException(new JuggleExceptionInternal(
	...
	private static String popupItems[] = { "Change title...",
			"Change overall timing...", "Add event to L hand",
	...
	protected void setupPopup() {
		...
		for (int i = 0; i < popupItems.length; i++) {
			...
			if (name != null) {
				item = new JMenuItem(guistrings.getString(name
						.replace(' ', '_')));
	...
	public void actionPerformed(ActionEvent event) {
		...
		for (int i = 0; i < popupItems.length; i++) {
			if (popupItems[i] != null
					&& name.equals(guistrings.getString(popupItems[i].replace(
							' ', '_')))) {
				itemnum = i;
				break;
			}
		...
		case 8: // Make last in event
		{
			...
			if (!ev.isMaster())
				ev = ev.getMaster();
			JMLTransition tr = ev
					.getTransition(((LadderEventItem) popupitem).transnum);
			ev.removeTransition(tr);
	...
	protected void changeTitle() {
		final JDialog jd = new JDialog(parent,
				guistrings.getString("Change_title"), true);
		...
		gb.setConstraints(
				tf,
				make_constraints(GridBagConstraints.LINE_START, 0, 0,
						new Insets(10, 10, 0, 10)));
		jd.getContentPane().add(okbutton);
		gb.setConstraints(
				okbutton,
				make_constraints(GridBagConstraints.LINE_END, 0, 1, new Insets(
						10, 10, 10, 10)));
	...
	protected void changeTiming() {
		final JDialog jd = new JDialog(parent,
				guistrings.getString("Change_timing"), true);
		GridBagLayout gb = new GridBagLayout();
		jd.getContentPane().setLayout(gb);
		...
		gb.setConstraints(
				lab,
				make_constraints(GridBagConstraints.LINE_END, 0, 0, new Insets(
						0, 0, 0, 0)));
		final JTextField tf = new JTextField(7);
		p1.add(tf);
		gb.setConstraints(
				tf,
				make_constraints(GridBagConstraints.LINE_START, 1, 0,
						new Insets(0, 5, 0, 0)));
	...
	protected JMLEvent addEventToHand(int hand) {
		...
		for (int i = 0; i < pat.getNumberOfPaths(); i++) {
			...
			while (evt != null) {
				JMLTransition tr = evt.getPathTransition(i + 1,
			...
			if (holding) {
				JMLTransition tr = new JMLTransition(
						JMLTransition.TRANS_HOLDING, (i + 1), null, null);
				ev.addTransition(tr);
			}
		}
	...
	protected void defineProp() {
		...
		// System.out.println("pathnum = " + pathnum + ", propnum = " +
		// propnum);
		...
		final JDialog jd = new JDialog(parent,
				guistrings.getString("Define_prop"), true);
		GridBagLayout gb = new GridBagLayout();
		jd.getContentPane().setLayout(gb);
		...
		gb.setConstraints(
				okbutton,
				make_constraints(GridBagConstraints.LINE_END, 1, 0, new Insets(
						0, 10, 0, 0)));
		okbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String type = (String) cb1.getItemAt(cb1.getSelectedIndex());
				...
				// check to see if any other paths are using this prop
				// definition
				...
				for (int i = 1; i <= pat.getNumberOfProps(); i++) {
					...
					if (type.equalsIgnoreCase(pdef.getType())) {
						if (((mod == null) && (pdef.getMod() == null))
								|| ((mod != null) && mod.equalsIgnoreCase(pdef
										.getMod()))) {
							gotmatch = true;
							matchingprop = i;
							break;
		...
		gb.setConstraints(
				p2,
				make_constraints(GridBagConstraints.LINE_START, 0, 1,
						new Insets(0, 0, 0, 0)));
		jd.getContentPane().add(p3);
		gb.setConstraints(
				p3,
				make_constraints(GridBagConstraints.LINE_END, 0, 2, new Insets(
						10, 10, 10, 10)));
		jd.getRootPane().setDefaultButton(okbutton);// OK button is default
	...
	protected void defineThrow() {
		...
		if (!ev.isMaster())
			ev = ev.getMaster();
		final JMLTransition tr = ev
				.getTransition(((LadderEventItem) popupitem).transnum);

		String[] pptypes = Path.builtinPaths;

		final JDialog jd = new JDialog(parent,
				guistrings.getString("Define_throw"), true);
		GridBagLayout gb = new GridBagLayout();
		jd.getContentPane().setLayout(gb);
	...
	protected void makeParametersPanel(JPanel jp, ParameterDescriptor[] pd) {
		...
		if (pd.length != 0) {
			 ...
			 * 
			 * jp.add(lab1); gb.setConstraints(lab1,
			 * make_constraints(GridBagConstraints.LINE_START,0,0, new
			 * Insets(10,10,0,10)));
			 */
			...
			for (int i = 0; i < pd.length; i++) {
				...
				gb.setConstraints(
						lab,
						make_constraints(GridBagConstraints.LINE_START, 0, i,
								new Insets(0, 0, 0, 0)));
				if (pd[i].type == ParameterDescriptor.TYPE_BOOLEAN) {
					...
					gb.setConstraints(
							jcb,
							make_constraints(GridBagConstraints.LINE_START, 1,
									i, new Insets(2, 5, 2, 0)));
					...
					// Clicking on the icon launches a file chooser for getting
					// a new image
					label.addMouseListener(new MouseAdapter() {
						public void mouseClicked(MouseEvent e) {
							...
							PlatformSpecific ps = PlatformSpecific
									.getPlatformSpecific();
							int result = ps.showOpenDialog(
									EditLadderDiagram.this, filter);
							if (result == JFileChooser.APPROVE_OPTION) {
								try {
									URL source = ps.getSelectedFile().toURI()
											.toURL();
									// We have to load the image to get the
									// correct dimensions
									...
									fpds[0].value = source;
									// fpds[1].value = new
									// Integer(icon.getIconWidth());
									// fpds[2].value = new
									// Integer(icon.getIconHeight());
									// fpds[1].default_value = fpds[1].value;
									// fpds[2].default_value = fpds[2].value;
									// Remake the parameter panal with new
									// default values.
									makeParametersPanel(fjp, fpds);
									((JDialog) (fjp.getTopLevelAncestor()))
											.pack();
								} catch (MalformedURLException ex) {
									// This should never happen
									ErrorDialog
											.handleException(new JuggleExceptionUser(
													errorstrings
															.getString("Error_malformed_URL.")));
								}
							}
						}
					});
		...
		for (int i = 0; i < dialog_pd.length; i++) {
			...
			if (dialog_pd[i].type == ParameterDescriptor.TYPE_BOOLEAN) {
				...
				try {
					...
					String template = errorstrings
							.getString("Error_number_format");
					Object[] arguments = { dialog_pd[i].name };
					throw new JuggleExceptionUser(MessageFormat.format(
							template, arguments));
				}
				...
				if (!icon.getDescription().equals(def))
					term = icon.getDescription(); // This contains the URL
													// string
			}
	...
	protected void layoutPattern() {
		try {
			...
			ErrorDialog.handleException(new JuggleExceptionInternal(jeu
					.getMessage()));
		} catch (JuggleExceptionInternal jei) {
			ErrorDialog.handleException(jei);
		}
	...
	protected void paintComponent(Graphics gr) {
		...
		// Could probably get this permutation from the pattern instead of
		// the animator.
		...
		for (int i = 0; i < laddereventitems.size(); i++) {
			...
			int yoffset = ((gui_state == STATE_MOVING_EVENT) && (active_eventitem.eventitem == item.eventitem)) ? delta_y
					: 0;
			if (item.type == item.TYPE_EVENT)
				gr.fillOval(item.xlow, item.ylow + yoffset,
						(item.xhigh - item.xlow), (item.yhigh - item.ylow));
			else {
				// This condition could probably be applied to all event
				// drawing.
				if (item.ylow + yoffset >= border_top
					...
					if (pathnum - 1 < animpropnum.length) {
		...
		if (active_eventitem != null) {
			...
			gr.drawLine(active_eventitem.xlow - 1, active_eventitem.ylow
					+ yoffset - 1, active_eventitem.xhigh + 1,
					active_eventitem.ylow + yoffset - 1);
			gr.drawLine(active_eventitem.xhigh + 1, active_eventitem.ylow
					+ yoffset - 1, active_eventitem.xhigh + 1,
					active_eventitem.yhigh + yoffset + 1);
			gr.drawLine(active_eventitem.xhigh + 1, active_eventitem.yhigh
					+ yoffset + 1, active_eventitem.xlow,
					active_eventitem.yhigh + yoffset + 1);
			gr.drawLine(active_eventitem.xlow - 1, active_eventitem.yhigh
					+ yoffset + 1, active_eventitem.xlow - 1,
					active_eventitem.ylow + yoffset - 1);
		}
	...
	/*
	 * private int orbits[][] = null; private Color orbitColor[] = null;
	 * 
	 * public void restartView(JMLPattern pat, AnimatorPrefs jc) throws
	 * JuggleExceptionUser, JuggleExceptionInternal { if (pat != null) this.pat
	 * = pat; if (jc != null) this.jc = jc;
	 * 
	 * if (pat != null) { OrbitLadderDiagram new_ladder = new
	 * OrbitLadderDiagram(pat, parent); new_ladder.setAnimator(jae);
	 * jae.setLadderDiagram(new_ladder); jae.deactivateEvent();
	 * new_ladder.setPreferredSize(new Dimension(ladder_width, 1));
	 * new_ladder.setMinimumSize(new Dimension(ladder_min_width, 1));
	 * this.ladder.removeAll(); this.ladder.add(new_ladder,
	 * BorderLayout.CENTER); this.ladder.validate();
	 * 
	 * findOrbits(); // Color the paths of the ladder diagram for (int i = 0; i
	 * < orbits.length; i++) { for (int j = 0; j < orbits[i].length; j++) {
	 * new_ladder.setPathColor(orbits[i][j]+1, orbitColor[i]); } } }
	 * 
	 * jae.restartJuggle(pat, jc); }
	 * 
	 * private void findOrbits() { if (this.pat == null) return;
	 * 
	 * int pathorb[] = new int[this.pat.getNumberOfPaths()]; int orbids[] = new
	 * int[this.pat.getNumberOfPaths()]; int orbitCount = 0;
	 * 
	 * for (int i = 0; i < pathorb.length ; i++) { pathorb[i] = -1; }
	 * 
	 * // Go through all the symetries and work out which path belongs // to
	 * which orbit. for (int i = 0; i < this.pat.getNumberOfSymmetries(); i++) {
	 * Permutation perm = this.pat.getSymmetry(i).getPathPerm(); for (int j = 0;
	 * j < perm.getSize(); j++) { int elem = perm.getMapping(j+1)-1; // Not zero
	 * based ! if (pathorb[elem] == -1) { if (pathorb[j] == -1) { pathorb[j] =
	 * orbitCount; orbids[orbitCount] = orbitCount; orbitCount++; }
	 * pathorb[elem] = pathorb[j]; } else { if (pathorb[j] == -1) { pathorb[j] =
	 * pathorb[elem]; } else if (pathorb[j] != pathorb[elem]) { // These were
	 * separate orbits, now should be merged orbids[pathorb[elem]] =
	 * orbids[pathorb[j]]; } } } }
	 * 
	 * // Clean up to have a sequential list int oc = 0; for (int i = 0; i <
	 * orbitCount; i++) { if (orbids[i]>=oc) { int v = orbids[i]; for (int j =
	 * i; j < orbitCount; j++) if (orbids[j] == v) orbids[j] = oc; oc++; } }
	 * 
	 * // Now store which orbit is made out of which paths this.orbits = new
	 * int[oc][]; for (int i = 0; i < oc; i++) { int ocp = 0; for (int j = 0; j
	 * < 2; j++) { for (int k = 0; k < pathorb.length; k++) { if
	 * (orbids[pathorb[k]] == i) { if (j == 1) this.orbits[i][ocp] = k; ocp++; }
	 * } if (j == 0) this.orbits[i] = new int[ocp]; ocp = 0; } }
	 * 
	 * // Now create the colors. We will never need more colors // than orbits.
	 * We want to exclude gray colors, as that // will be used for 'deselected'.
	 * this.orbitColor = new Color[oc]; int cols[] = {255,0,0}; int phase = 0;
	 * for (int i = 0; i < oc; i++) { this.orbitColor[i] = new Color(cols[0],
	 * cols[1], cols[2]);
	 * 
	 * if ((phase == 0 && cols[2] < 255) || (phase == 1 && cols[2] == 255)) {
	 * int d = cols[0]; cols[0] = cols[2]; cols[2] = cols[1]; cols[1] = d; }
	 * else if (phase == 0) { cols[1] = 255; phase++; } else if (phase == 1) {
	 * if (cols[2]>0) cols[1] = cols[2]/2; else cols[1] = cols[1]/2; cols[2] =
	 * cols[1]; phase = 0; } }
	 */

	/*
	 * System.out.println("there are " + oc + " orbits"); String blabel = "";
	 * for (int j = 0; j < this.orbits.length; j++) { blabel += "("; for (int k
	 * = 0; k < this.orbits[j].length; k++) { blabel += (this.orbits[j][k]+1);
	 * if (k < this.orbits[j].length-1) blabel += ","; } blabel += ")"; }
