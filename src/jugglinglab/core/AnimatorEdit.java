...
public class AnimatorEdit extends Animator {
	...
	public AnimatorEdit(View parentView) {
		super(parentView);
	}
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent me) {
				...
				if (event_active) {
					int mx = me.getX();
					int my = me.getY();
					if ((mx >= xlow1) && (mx <= xhigh1) && (my >= ylow1)
							&& (my <= yhigh1)) {
						dragging = true;
						dragging_left = true;
						xstart = mx;
			...
			public void mouseReleased(MouseEvent me) {
				...
				if (event_active && dragging) {
					JMLEvent master = (event.isMaster() ? event : event
							.getMaster());
					...
					if (AnimatorEdit.this.jc.stereo) {
						...
						newgc.setCoordinate(0.5 * newgc.x, 0.5 * newgc.y,
								0.5 * newgc.z);
					}
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent me) {
				...
				if (dragging) {
					...
					AnimatorEdit.this.camangle = AnimatorEdit.this.ren1
							.getCameraAngle();
				}
	...
	protected void createEventView() {
		if (event_active) {
			// translate by one pixel and see how far it was in juggler space
			{
				...
				int boxhw = (int) (0.5 + 5.0 / dl); // pixels corresponding to
													// 5cm in juggler space
	...
	protected void drawFrame(double sim_time, int[] pnum, Graphics g)
		...
		g2.drawLine(xlow1 + xdelta, ylow1 + ydelta, xhigh1 + xdelta, ylow1
				+ ydelta);
		g2.drawLine(xhigh1 + xdelta, ylow1 + ydelta, xhigh1 + xdelta, yhigh1
				+ ydelta);
		g2.drawLine(xhigh1 + xdelta, yhigh1 + ydelta, xlow1 + xdelta, yhigh1
				+ ydelta);
		g2.drawLine(xlow1 + xdelta, yhigh1 + ydelta, xlow1 + xdelta, ylow1
				+ ydelta);
		...
		if (this.jc.stereo) {
			...
			g2.setColor(Color.green);
			g2.drawLine(xlow2 + xdelta, ylow2 + ydelta, xhigh2 + xdelta, ylow2
					+ ydelta);
			g2.drawLine(xhigh2 + xdelta, ylow2 + ydelta, xhigh2 + xdelta,
					yhigh2 + ydelta);
			g2.drawLine(xhigh2 + xdelta, yhigh2 + ydelta, xlow2 + xdelta,
					yhigh2 + ydelta);
			g2.drawLine(xlow2 + xdelta, yhigh2 + ydelta, xlow2 + xdelta, ylow2
					+ ydelta);
		}
	}
