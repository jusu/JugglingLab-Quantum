...
public class ApplicationWindow extends JFrame implements ActionListener,
	...
	public ApplicationWindow(String title) throws JuggleExceptionUser,
		...
		ng.getDefaultButton().doClick();
	}

	protected static final String[] fileItems = new String[] { "Open JML...",
			"Convert JML...", null, "Quit" };
	protected static final String[] fileCommands = new String[] { "open",
			"convert", null, "exit" };
			...
			if (fileItems[i] == null)
				...
				if (fileShortcuts[i] != ' ')
					fileitem.setAccelerator(KeyStroke.getKeyStroke(
							fileShortcuts[i], Toolkit.getDefaultToolkit()
	...
	public void actionPerformed(ActionEvent ae) {
		...
		try {
			 ...
			 * doFileMenuCommand(FILE_NEWPAT); else if
			 * (command.equals("newlist")) doFileMenuCommand(FILE_NEWLIST); else
			 * if (command.equals("juggleanim")) doMenuCommand(FILE_CONVERT);
			 */
	...
	public void doMenuCommand(int action) throws JuggleExceptionInternal {
		...
		/*
		 * case FILE_NEWPAT: { PatternWindow jaw2 = null;
		 * 
		 * try { siteswapNotation sn = new siteswapNotation();
		 * 
		 * JMLPattern pat = sn.getJMLPattern("pattern=5"); jaw2 = new
		 * PatternWindow(pat.getTitle(), pat, new AnimatorPrefs()); } catch
		 * (JuggleExceptionUser je) { if (jaw2 != null) jaw2.dispose(); new
		 * ErrorDialog(this, je.getMessage()); } catch (JuggleException je) { if
		 * (jaw2 != null) jaw2.dispose(); throw new
		 * JuggleExceptionInternal(je.getMessage()); } } break;
		 * 
		 * case FILE_NEWLIST: new
		 * PatternListWindow(guistrings.getString("Pattern_List")); break;
		 */
		...
		case FILE_OPEN: {
			...
			try {
				if (PlatformSpecific.getPlatformSpecific().showOpenDialog(this,
						filter) == JFileChooser.APPROVE_OPTION) {
					if (PlatformSpecific.getPlatformSpecific()
							.getSelectedFile() != null)
						showJMLWindow(PlatformSpecific.getPlatformSpecific()
								.getSelectedFile());
				}
		...
		/*
		 * case FILE_JUGGLEANIM: { PatternListWindow pw = null;
		 * 
		 * try { try { int option =
		 * PlatformSpecific.getPlatformSpecific().showOpenDialog(this); if
		 * (option == JFileChooser.APPROVE_OPTION) { if
		 * (PlatformSpecific.getPlatformSpecific().getSelectedFile() != null) {
		 * pw = new PatternListWindow(guistrings.getString("Patterns"));
		 * readJuggleAnimPatternfile(pw, new
		 * FileReader(PlatformSpecific.getPlatformSpecific
		 * ().getSelectedFile())); } } } catch (FileNotFoundException fnfe) {
		 * throw new
		 * JuggleExceptionUser(errorstrings.getString("Error_file_not_found")+
		 * ": "+fnfe.getMessage()); } catch (IOException ioe) { throw new
		 * JuggleExceptionUser
		 * (errorstrings.getString("Error_IO")+": "+ioe.getMessage()); } } catch
		 * (JuggleExceptionUser jeu) { if (pw != null) pw.dispose(); new
		 * ErrorDialog(this, jeu.getMessage()); } catch (JuggleExceptionInternal
		 * jei) { if (pw != null) pw.dispose();
		 * ErrorDialog.handleException(jei); } break; }
		 */
		...
		case FILE_CONVERT: {
			...
			try {
				if (PlatformSpecific.getPlatformSpecific().showOpenDialog(this,
					...
					if (PlatformSpecific.getPlatformSpecific()
							.getSelectedFile() != null)
						convertJML(PlatformSpecific.getPlatformSpecific()
								.getSelectedFile());
				}
			} catch (JuggleExceptionUser je) {
				new ErrorDialog(this, je.getMessage());
	...
	public void showJMLWindow(File jmlf) throws JuggleExceptionUser,
			JuggleExceptionInternal {
		JFrame frame = null;
		...
		try {
			try {
				...
				case JMLParser.JML_PATTERN: {
					...
					frame = new PatternWindow(pat.getTitle(), pat,
							new AnimatorPrefs());
					break;
				}
				...
				throw new JuggleExceptionUser(
						errorstrings.getString("Error_file_not_found") + ": "
								+ fnfe.getMessage());
			} catch (IOException ioe) {
				throw new JuggleExceptionUser(
						errorstrings.getString("Error_IO") + ": "
								+ ioe.getMessage());
	...
	public void convertJML(File jmlf) throws JuggleExceptionUser {
		...
		// check if we should be loading as JML version 1.0 (switch y and z
		// coordinates in events)
		boolean switchyz = false;
		Matcher m = Pattern.compile(
				"<jml[^>]*version[\\s]*=[\\s]*['\"]([^'\"]*)['\"]").matcher(in);
		if (m.find()) {
			if (m.group(1).equals("1.0"))
				switchyz = true;
			else if (m.group(1).equals("1.2"))
				throw new JuggleExceptionUser(
						errorstrings.getString("Error_already_12"));
		...
		while (m.find()) {
			String g = "<title>" + JMLNode.xmlescape(m.group(1)).trim()
					+ "</title>";
		...
		// <line (...1) notation="jml" pattern='(...2)'(...3)/> ---> <line
		// (...1) notation="jml"(...3)>(...2)</line>
		sb = new StringBuffer();
		m = Pattern
				.compile(
						"(<line[^>]*notation[\\s]*=[\\s]*\"jml\"[^>]*?)[\\s]+pattern[\\s]*=[\\s]*['\"]([^']*)['\"]([^>]*)/>")
				.matcher(out);
		while (m.find()) {
		...
		// <line (...1) pattern='(...2)' notation="jml" (...3)/> ---> <line
		// (...1) notation="jml"(...3)>(...2)</line>
		sb = new StringBuffer();
		m = Pattern
				.compile(
						"(<line[^>]*?)[\\s]+pattern[\\s]*=[\\s]*['\"]([^']*)['\"]([^>]*notation[\\s]*=[\\s]*\"jml\"[^>]*?)[\\s]*/>")
				.matcher(out);
		while (m.find()) {
			String g = m.group(1) + m.group(3) + ">" + m.group(2) + "</line>";
			m.appendReplacement(sb, g);
		...
		// all of the JML patterns are taken care of above; any remaining lines
		// with
		// 'pattern=' are in other notations

		// <line (...1) pattern='(...2)' (...3)/> ---> <line (...1)
		// (...3)>XMLc(...2)</line>
		sb = new StringBuffer();
		m = Pattern
				.compile(
						"(<line[^>]*?)[\\s]+pattern[\\s]*=[\\s]*['\"]([^'\"]*)['\"]([^>]*?)[\\s]*/>")
				.matcher(out);
		while (m.find()) {
		...
		if (switchyz) {
			// <event (...1) y="(...2)" (...3) z="(...4)" ---> <event (...1)
			// y="(...4)" (...3) z="(...2)"
			sb = new StringBuffer();
			m = Pattern
					.compile(
							"(<event[^>]*)[\\s]+([yz])[\\s]*=[\\s]*\"([^\"]*)\"([^>]*)[\\s]+([yz])[\\s]*=[\\s]*\"([^\"]*)\"")
					.matcher(out);
			while (m.find()) {
				String g = m.group(1) + " " + m.group(2) + "=\"" + m.group(6)
						+ "\"" + m.group(4) + " " + m.group(5) + "=\""
						+ m.group(3) + "\"";
				m.appendReplacement(sb, g);
			}
		...
		if (m.find())
			outname = m.group(1) + "_converted" + m.group(2); // insert before
																// filename
																// extension
		else
		...
		try {
			...
			if (outfile.exists()) {
				String template = errorstrings
						.getString("Error_already_exists");
